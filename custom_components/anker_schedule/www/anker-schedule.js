/*
 * Anker Schedule — tiny Lovelace stub.
 *
 * 1) Registreert `anker-schedule` meteen (HA's ~2s timeout).
...
console.info(`ANKER-SCHEDULE stub ${VERSION}`, CARD_URL);
...
window.__ankerScheduleRebuild = rebuildLovelace;
window.__ankerScheduleActivate = activate;
...
const defineElement = (name, ctor) => {
  ...
  const heal = (via) => {
    ...
    try {
      ...
      console.info(
        `ANKER-SCHEDULE: re-defined ${name} after customElements registry swap (${via}, frontend#52960)`
      );
      activate();
    } catch (err) {
      console.warn(
        `ANKER-SCHEDULE: re-define ${name} failed (${via})`,
        err
      );
    }
  };
...
const loadingHtml =
  ...
  '<div style="color:var(--primary-text-color,#eaf6ff);font-size:13px;font-weight:600;letter-spacing:1px">ANKER PLANNER</div>' +
  '<div style="color:var(--secondary-text-color,#7fa6b8);font-size:11px;margin-top:8px">Card wordt geladen…</div>' +
  "</ha-card>";

class AnkerScheduleBootstrap extends HTMLElement {
  static getConfigElement() {
    return document.createElement(EDITOR);
  }

  static getStubConfig() {
    return { title: "ANKER PLANNER" };
  }
...
class AnkerScheduleEditorBootstrap extends HTMLElement {
  setConfig(config) {
    this._cfg = config || {};
    this._ensure();
  }
...
defineElement(TAG, AnkerScheduleBootstrap);
defineElement(EDITOR, AnkerScheduleEditorBootstrap);
...
if (!window.__ankerScheduleCardLoading) {
  window.__ankerScheduleCardLoading = true;
  import(CARD_URL)
    .then(() => {
      window.__ankerScheduleCardLoaded = true;
      activate();
    })
    .catch((err) => {
      console.error(
        "Anker Schedule: laden van card-module mislukt",
        CARD_URL,
        err
      );
      walkShadow(document, (root) => {
        root.querySelectorAll?.(TAG).forEach((el) => {
          el.innerHTML =
            ...
            "Anker Schedule card kon niet laden. Hard refresh of HA herstarten." +
            "</ha-card>";
        });
      });
    });
} else if (window.__ankerScheduleCardLoaded) {
  activate();
}
