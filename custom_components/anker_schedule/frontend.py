...
def _copy_card_to_local_www(hass: HomeAssistant, src_www: Path) -> Path | None:
    """Copy card + logo into config/www/anker-schedule for /local/."""
    ...
    try:
        ...
        stale = dest_dir / "anker-schedule-card.js"
        if stale.is_file():
            try:
                stale.unlink()
            except OSError:
...
async def async_register_frontend(hass: HomeAssistant) -> None:
    ...
    if not card_path.is_file():
        _LOGGER.error(
            "Anker Schedule cardbestand ontbreekt (verwacht %s)",
            card_path,
        )
        return
    ...
    _LOGGER.info(
        "Anker Schedule card op %s via extra_module_url (local copy: %s)",
        primary_url,
...
async def _async_remove_lovelace_resources(hass: HomeAssistant) -> None:
    ...
    async def _try_remove() -> None:
        ...
        removed = 0
        for item in items:
            if not _is_anker_resource_url(str(item.get("url", ""))):
                continue
            try:
                await resources.async_delete_item(item["id"])
                removed += 1
                _LOGGER.info(
                    "Dubbele Anker Lovelace-resource verwijderd: %s",
                    item.get("url"),
                )
            except Exception:  # noqa: BLE001
                _LOGGER.warning(
        ...
        if removed:
            _LOGGER.info(
                "Anker Schedule: %s dubbele resource(s) opgeruimd", removed
            )
