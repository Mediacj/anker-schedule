"""Anker Schedule Home Assistant integration."""
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    """Set up the Anker Schedule integration (frontend once)."""
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up Anker Schedule from a config entry."""
    await async_register_frontend(hass)

    coordinator = AnkerScheduleCoordinator(hass, entry)
    hass.data.setdefault(DOMAIN, {})
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if unload_ok:
        coordinator: AnkerScheduleCoordinator | None = hass.data[DOMAIN].pop(
            entry.entry_id, None
        )
...
def _async_register_services(hass: HomeAssistant) -> None:
    ...
    async def handle_apply_now(call: ServiceCall) -> None:
        ...
        for key, coordinator in hass.data.get(DOMAIN, {}).items():
            if not isinstance(coordinator, AnkerScheduleCoordinator):
                continue
